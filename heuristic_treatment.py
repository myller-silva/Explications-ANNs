...
import shap
from teste import insert_output_constraints_tjeng, get_minimal_explanation
...
def refine_heuristic_explanation(mdl, heuristic_explanation, network_input, network_output, n_classes, output_bounds):
    return get_minimal_explanation(mdl, tf.constant(network_input), network_output, n_classes=n_classes,
...
if __name__ == '__main__':
    ...
    for i in range(data.shape[0]):
        ...
        start = time()
        explanation = get_minimal_explanation(mdl.clone(), tf.constant(network_input), predicted_class,
